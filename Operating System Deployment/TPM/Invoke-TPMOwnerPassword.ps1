...
.EXAMPLE
    ...
    Revert TPM Owner Password behavior to Windows 10 version 1607 and later:
...
.NOTES
    ...
    1.0.1 - (2018-04-06) Updated script with more logging and validation options and extended the Test-RegistryValue function
#>
...
Process {
    ...
    Write-CMLogEntry -Value "Running in $($Mode) operational mode" -Severity 1
        ...
        "Save" {
            ...
            if ($OAFTSVariable -ne $null) {
                ...
                try {
                    ...
                    Write-CMLogEntry -Value "Existence of TPM key location: $(Test-RegistryValue -Path $TPMKeyPath)" -Severity 1
                    if (-not(Test-RegistryValue -Path $TPMKeyPath)) {
                        New-Item -Path $TPMKeyPath -Force -ErrorAction Stop
                        Write-CMLogEntry -Value "Successfully created TPM key location" -Severity 1
                    }
                ...
                # Validate OSManagedAuthLevel registry value
                try {
                    $OSManagedAuthLevelValue = Get-ItemProperty -Path "HKLM:\Software\Policies\Microsoft\TPM" -Name OSManagedAuthLevel -ErrorAction Stop | Select-Object -ExpandProperty OSManagedAuthLevel
                    Write-CMLogEntry -Value "Current value for OSManagedAuthLevel: $($OSManagedAuthLevelValue) (should be 4)" -Severity 1
                }
                catch [System.Exception] {
                    Write-CMLogEntry -Value "An error occured when attempting to validate the OSManagedAuthLevel value in the registry. Error message: $($_.Exception.Message)" -Severity 3
                }                
            }
        }
        "Revert" {
            ...
            try {
                Set-ItemProperty -Path "HKLM:\Software\Policies\Microsoft\TPM" -Name OSManagedAuthLevel -Value 2 -Force -ErrorAction Stop
            ...
            try {
                $OSManagedAuthLevelValue = Get-ItemProperty -Path "HKLM:\Software\Policies\Microsoft\TPM" -Name OSManagedAuthLevel -ErrorAction Stop | Select-Object -ExpandProperty OSManagedAuthLevel
                Write-CMLogEntry -Value "Current value for OSManagedAuthLevel: $($OSManagedAuthLevelValue) (should be 2)" -Severity 1
            }
            catch [System.Exception] {
                Write-CMLogEntry -Value "An error occured when attempting to validate the OSManagedAuthLevel value in the registry. Error message: $($_.Exception.Message)" -Severity 3
            }
    ...
    Write-CMLogEntry -Value "Completed TPM Owner Password control" -Severity 1
}
